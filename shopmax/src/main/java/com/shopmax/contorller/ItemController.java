...
public class ItemController {
    ...
    public String itemForm(Model model) {
        //valid 체크를 위해 비어있는 DTO 객체를 꼭 넣어준다.
    ...
    public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult, Model model,
        ...
        if(bindingResult.hasErrors()) return "item/itemForm"; //유효성 체크에서 걸리면 itemForm으로 이동
        ...
        return "redirect:/"; //상품등록이 문제 없이 완료되면 index화면으로 이동
    }
    ...
    public String itemManage(ItemSearchDto itemSearchDto,
        ...
        //PageRequest.of(페이지 번호(0부터 시작), 한페이지당 조회할 데이터 갯수);
        //URL path에 페이지가 있으면 해당 페이지 번호를 조회하고
        // 페이지 번호가 없다면 0페이지(첫번째 페이지)를 조회
    ...
    public String itemShopList(Model model, ItemSearchDto itemSearchDto,
                               //@RequestParam Get방식이라 input hidden등으로 가져옴.
